...
class ShowTimeExtractor:
	...
	def get_times(self):
		...
		for k in city_lookup:
			if self.city.lower() in k.lower():
				MainCity = k
				break

		if MainCity:
			url_tag = city_lookup[MainCity]
	...
	def json_to_html(self, theaters):
		...
		for each in theaters:
			if each['theater_data'] != None:
				
				if 'phone' not in each['theater_data']:
					phoneDiv = ""
				else:
					phoneDiv = """<div class="theater-Info">
									<div class="row">
									<div class="col-md-1 col-sm-1 col-xs-1"><i class="fa fa-phone"></i></div>
									<div class="col-md-10 col-sm-10 col-xs-10"><p>""" + each['theater_data']['phone'].replace("Phone:","").split(",")[0] + """</p></div>
									</div>
								</div>"""


				if 'address' not in each['theater_data']:
					addrDiv = ""
				else:
					addrDiv = """<div class="theater-Info">
									<div class="row">
			...
			ctime = "<p>"
			for daydic in each['times'][:4]:
				for xx, yy in daydic.iteritems():
					ctime += """<p><span class="day-time">""" + xx.split(",")[0] + """</span></p>"""
					for tt in yy:
			...
			row = """<div class="row-card">
				<div class="stack-show ">
					<div class="theater-id col-md-5">
						<div class="theater-name">""" + fname +  """ <p class='lname'>""" + lname + """</p>  </div>
						""" + addrDiv + """
						""" + phoneDiv + """
						</div>
...
# a = ShowTimeExtractor('Noida' , 'http://www.moviebuff.com/hate-story-3').get_times()
# print a 